...
async fn main() -> Result<()> {
    println!("\x1b[96mCreating directories\x1b[0m");
    create_directories()?;
...
async fn download_files(regions_delegated: &HashMap<&str, &str>) -> Result<()> {
    ...
    let download_tasks: Vec<_> = regions_delegated
        ...
        .map(|(key, url)| {
            ...
            async move {
                println!("\x1b[96mDownloading {}\x1b[0m", key_str);
...
async fn process_files(
    ...
    let process_tasks: Vec<_> = regions_delegated
        ...
        .map(|key| {
            ...
            async move {
                println!("\x1b[95m{}\x1b[0m", key_str);
